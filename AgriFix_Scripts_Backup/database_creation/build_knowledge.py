...
def _image_db_to_chunks(json_path: Path) -> List[ParsedChunk]:
    ...
    for img in data:
        ...
        related_block = ""
        if related_images:
            lines = [f"  - {r['filename']} (sim={r['similarity']:.2f}): {r['caption']}" for r in related_images]
        ...
        tags = _flatten_list(img.get("search_keywords", [])) + \
               [diagram_type_canon, diagram_type_raw, img.get("section_context", "")] + \
               figure_refs
...
def _flatten_list(value) -> List[str]:
    """Helper to safely flatten nested lists or strings into a flat list of strings."""
    out = []
